...
class _FakeRecSession:
    ...
    instances: list["_FakeRecSession"] = []

    def __init__(self, session, device_id, credential, on_update):
        ...
        self.closed = False
        self.stopped = False
        _FakeRecSession.instances.append(self)
...
async def test_turning_on_holds_a_session_and_reflects_its_real_state(hass):
    ...
    assert state.attributes["origin"] == "manual"
    assert len(_FakeRecSession.instances) == 1
...
async def test_turning_off_stops_the_held_session(hass):
    ...
    assert _FakeRecSession.instances[-1].stopped


async def test_the_doorbell_ending_the_recording_turns_the_switch_off_by_itself(hass):
    """Point 2 of the task: the exposed state is the doorbell's real `rec_state`, never
    ...
    with patch("custom_components.ig_doorbell.switch.RecSession", _FakeRecSession):
        ...
        session = _FakeRecSession.instances[-1]
        session.push_end()
        await hass.async_block_till_done()
        assert hass.states.get(ent).state == "off"
        assert not session.closed  # still held at this instant - only `recording` changed

        await session.stop()      # what the real RecSession schedules next, to free the slot
        await hass.async_block_till_done()
...
async def test_unloading_the_entry_closes_an_open_rec_session_no_leak(hass):
    """Point 1's "no leaks": a Home Assistant restart or a reload must not leave the doorbell
