...
class _Portero:
    ...
    def __init__(self) -> None:
        self.estado = {"m": 0, "door_m": 0, "webrtc_clients": 1, "dname": "Test"}
        self.ignorar_escrituras = False

    async def get_states(self, *_a, **_k):
        return dict(self.estado)

    async def save_states(self, _sesion, _device_id, _credential, campos):
        if not self.ignorar_escrituras:
            self.estado["m"] = int(campos["m"])


async def _montar(hass, portero: _Portero):
    ...
    with patch.object(net, "es_este_portero", AsyncMock(return_value=True)), \
         patch.object(api, "async_get_states", AsyncMock(side_effect=portero.get_states)), \
    ...
    return er.async_get(hass).async_get_entity_id("select", DOMAIN, f"{DEVICE_ID}_modo")


async def _elegir(hass, ent, opcion, portero):
    with patch.object(api, "async_get_states", AsyncMock(side_effect=portero.get_states)), \
         patch.object(api, "async_save_states", AsyncMock(side_effect=portero.save_states)):
        await hass.services.async_call(
            "select", "select_option", {"entity_id": ent, "option": opcion}, blocking=True
        )


async def test_back_to_back_changes_show_at_once(hass):
    portero = _Portero()
    ent = await _montar(hass, portero)
    assert hass.states.get(ent).state == "normal"

    await _elegir(hass, ent, "away", portero)
    ...
    await _elegir(hass, ent, "do_not_disturb", portero)
...
async def test_a_change_the_doorbell_dropped_fails_and_keeps_the_real_mode(hass):
    portero = _Portero()
    ent = await _montar(hass, portero)
    portero.ignorar_escrituras = True
